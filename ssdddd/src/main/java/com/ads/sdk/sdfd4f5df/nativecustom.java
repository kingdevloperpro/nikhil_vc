...
public class nativecustom {
    ...
    public void showMyCustomNative(final Activity activity, ViewGroup nativeAdContainer, int type) {
        if (new fsdfdsf445(activity).getCustomAdshowStatus().equals("0") || new fsdfdsf445(activity).getCustomNative().equals("0")) {
            return;
        }
        addatacustom appModal = fggfg.getMyCustomAd();
    ...
    private View getCustomView(Activity activity, ViewGroup nativead, int type) {
        ...
        switch (type) {
            case 1:
                adView = activity.getLayoutInflater().inflate(R.layout.htghtgf, null);
                break;
            case 2:
                adView = activity.getLayoutInflater().inflate(R.layout.dgf3fdg3, null);
                break;
            default:
                adView = activity.getLayoutInflater().inflate(R.layout.htghtgf, null);
                break;

...
import static com.ads.sdk.fggfg.getMyCustomAd;
...
public class intads_activity extends AppCompatActivity {
    ...
    public static void newIntent(Activity activity, g1fg1 listner_for_app) {
        if (new fsdfdsf445(activity).getCustomAdshowStatus().equals("0") || new fsdfdsf445(activity).getCustomInter().equals("0")) {
            return;
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.egergg236);
        if (new fsdfdsf445(this).getCustomAdshowStatus().equals("0") || new fsdfdsf445(this).getCustomInter().equals("0")) {
            return;
        }
    ...
    private void closeAd(Activity activity, int pos) {
        if (new fsdfdsf445(activity).getCustomCloseAd().equals("1")) {
            clicked = true;
            action_str = customAdData.get(pos).getUrl();
            if (action_str.startsWith("https")) {
                startActivity(new Intent("android.intent.action.VIEW", Uri.parse(getPlayStoreUrl(action_str))));

package com.pinky.videochat.globalcall.morli
...
import kotlinx.android.synthetic.main.sdgdfgfdg.bkk
import kotlinx.android.synthetic.main.sdgdfgfdg.button_pay_bronze
import kotlinx.android.synthetic.main.sdgdfgfdg.button_pay_gold
import kotlinx.android.synthetic.main.sdgdfgfdg.button_pay_silver
import kotlinx.android.synthetic.main.sdgdfgfdg.payId
import kotlinx.android.synthetic.main.sdgdfgfdg.paymentView
import kotlinx.android.synthetic.main.sdgdfgfdg.successView
import kotlinx.android.synthetic.main.sdgdfgfdg.txt_amt_bronze
import kotlinx.android.synthetic.main.sdgdfgfdg.txt_amt_gold
import kotlinx.android.synthetic.main.sdgdfgfdg.txt_amt_silver
...
class MainActivity : AppCompatActivity(), PaymentStatusListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.sdgdfgfdg)
    ...
    private fun setPrice() {
        txt_amt_gold.text = "₹ " + fsdfdsf445(this).keyInAppPriceGold + "/-"
        txt_amt_silver.text = "₹ " + fsdfdsf445(this).keyInAppPriceSilver + "/-"
        txt_amt_bronze.text = "₹ " + fsdfdsf445(this).keyInAppPriceBronze + "/-"
    }
    ...
    private fun pay(premium_key_type: String) {
        ...
        if (premium_key_type == KEY_GOLD) {
            amount = fsdfdsf445(this).keyInAppPriceGold
        } else if (premium_key_type == KEY_SILVER) {
            amount = fsdfdsf445(this).keyInAppPriceSilver
        } else if (premium_key_type == KEY_BRONZE) {
            amount = fsdfdsf445(this).keyInAppPriceBronze
        } else {
            Toast.makeText(this, "something went wrong!", Toast.LENGTH_SHORT)
        }


        val payeeVpa = fsdfdsf445(this).keyInAppUpiId
        val payeeName = fsdfdsf445(this).keyInAppUpiName
        val transactionId = "TID" + System.currentTimeMillis()
        val transactionRefId = "TID" + System.currentTimeMillis()
        val payeeMerchantCode = "";
        val description = fsdfdsf445(this).keyInAppUpiDesc
    ...
    private fun onTransactionSuccess() {
        ...
        f4sdf48sd5f4.setVipType(glob_premium_key_type)
        successPay()
    }
...
//        toast("Pending | Submitted")
    ...
    private fun manageVip() {
        ...
        if (f4sdf48sd5f4.getVipType() == "in_app_gold") {
            fsdfdsf445(this).setAdsFlag(false)
            fsdfdsf445(this).redeemGoldPlan()
            fsdfdsf445(this).set_IncomingWelcome_Fix()
        } else if (f4sdf48sd5f4.getVipType() == "in_app_silver") {
            fsdfdsf445(this).setAdsFlag(false)
            fsdfdsf445(this).redeemSilverPlan()
        } else if (f4sdf48sd5f4.getVipType() == "in_app_bronze") {
            fsdfdsf445(this).redeemBronzePlan()
            val sh = getSharedPreferences("RandomSF", MODE_APPEND)
            val vc_count = sh.getInt("KEY_RAND", 0)
            if (vc_count == 7) {
                f4sdf48sd5f4.setVipType("null")
                Toast.makeText(
                    this,
